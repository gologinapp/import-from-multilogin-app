...
class Auxiliary:
    ...
    def get_proxy(self, proxy_type):
        if proxy_type == 'no_proxy':
            print("Profile without proxy")
            return {}
        elif proxy_type != 'no_proxy' and not('proxyUser' in self.result):
            print(f"Profile without auth, proxy mode {proxy_type}")
            return {
                'mode': proxy_type,
            ...
            print(f"Profile with auth, proxy mode {proxy_type}")
            return {
                'mode': proxy_type,
                'host': self.result['proxyHost'],
                'port': self.result['proxyPort'],
